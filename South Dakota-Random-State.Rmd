---
title: 'COVID-19 Random State: Tennessee'
...
VI <- dbGetQuery(db,"select date,state,TotalCases,TotalDeaths,new_cases,new_deaths from  STATESDAILY where state='Tennessee'")
VI$date <- as.Date(VI$date)
VI$date <- sort(VI$date,decreasing = TRUE)
str(VI)

## County Data
DF <- dbGetQuery(db,"select date,state,county,new_cases,new_deaths from  COUNTYDAILY where state='Tennessee'")
...
### Top 5 countries by Single day totals for Aug. 30, 2020

```{r}
df <- subset(DF,date =="2020-09-02" )
Top <- df[order(df$new_cases,decreasing = TRUE),]
Top5 <-head(Top,n=5)
barplot(new_cases ~county,data=Top5,main="Top 5 Counties Ranked by single Day Cases",xlab="(For Sept. 2, 2020)",ylab="Cases")
grid(col="black")

Topd <- df[order(df$new_deaths,decreasing = TRUE),]
Top5d <-head(Topd,n=5)
barplot(new_deaths ~county,data=Top5d,main="Top 5 Counties Ranked by single Day Deaths",xlab="(For Sept. 2, 2020)",ylab="Cases")
grid(col="black")
```
