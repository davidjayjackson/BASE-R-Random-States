---
title: 'COVID-19 Random State: Arkansas'
...
VI <- dbGetQuery(db,"select date,state,TotalCases,TotalDeaths,new_cases,new_deaths from  STATESDAILY where state='Arkansas'")
...
```{r}
DF <- dbGetQuery(db,"select date,state,county,new_cases,new_deaths from  COUNTYDAILY where state='Arkansas'")
...
barplot(new_deaths ~county,data=df_deaths,main="Top 5 States Ranked by single Day Deaths",xlab="(For August 26, 2020)",ylab="Cases")
grid(col="black")
```
